...
const SearchResults = () => {
    ...
    useEffect(() => {
        if (query) {
            ...
            const searchInvoices = api.get(`/invoices/search?query=${encodeURIComponent(query)}`)
                .then(response => response.data.invoices || response.data || [])
                .catch(error => {
            ...
            const searchOrders = api.get(`/orders/search?query=${encodeURIComponent(query)}`)
        ...
        return (
            ...
            >
                ...
                <div className="space-y-2">
                    {type === 'project' && (
                        <>
                            <h3 className="text-xl font-bold truncate">{item.name || 'לא זמין'}</h3>
                            <p className="text-white/90">
                                תקציב: {item.budget ? `₪${formatNumber(item.budget)}` : 'לא הוגדר'}
                            </p>
                    ...
                    {type === 'invoice' && (
                        <>
                            <h3 className="text-xl font-bold">חשבונית מס׳ {item.invoiceNumber || 'לא זמין'}</h3>
                            <p className="text-white/90">
                            ...
                            {item.paid && (
                                <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${
                                    item.paid === 'כן' ? 'bg-green-500/20 text-green-100' : 'bg-red-500/20 text-red-100'
                                }`}>
                    ...
                    {type === 'order' && (
                        <>
                            <h3 className="text-xl font-bold">הזמנה מס׳ {item.orderNumber || 'לא זמין'}</h3>
                            <p className="text-white/90">
                                סכום: {item.sum ? `₪${formatNumber(item.sum)}` : 'לא זמין'}
                            </p>
                            <p className="text-white/80 text-sm truncate">
    ...
    const renderResults = () => {
        ...
        if (loading) {
            return (
                <div className="flex flex-col justify-center items-center h-64">
                    ...
                    <h1 className="mt-4 font-bold text-2xl text-cyan-950">טוען תוצאות חיפוש...</h1>
                </div>
            );
        }
        ...
        if (totalResults === 0 && !hasErrors) {
            return (
                <div className="text-center p-8 bg-white rounded-2xl shadow-xl">
                    <Search className="w-16 h-16 mx-auto mb-4 text-slate-400" />
                    <p className="text-xl text-slate-600 mb-2">לא נמצאו תוצאות עבור "{query}"</p>
                    <p className="text-sm text-slate-400">נסה לחפש במילים אחרות או בדוק את האיות</p>
                </div>
            );
        }
