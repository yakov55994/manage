...
export default function ExpenseLinkModal({
  ...
  const [amountWarningModal, setAmountWarningModal] = useState({ open: false, diff: 0, selected: 0, expenseAmt: 0 });
  ...
  useEffect(() => {
    if (open && expense) {
      const invIds = expense.linkedInvoices?.map(inv => inv._id || inv) || [];
      const salIds = expense.linkedSalaries?.map(sal => sal._id || sal) || [];
      const ordIds = expense.linkedOrders?.map(ord => ord._id || ord) || [];
  ...
  const amountMatch = selectedInvoiceIds.length === 0 && selectedSalaryIds.length === 0 && selectedOrderIds.length === 0
    ? null // אין בחירה
    : Math.abs(selectedTotal - expenseAmount) <= 0.01;
  ...
  const handleSave = () => {
    ...
    const totalSelected = selectedTotal; // מחושב ב-useMemo למטה
    const expenseAmount = parseFloat(expense.amount) || 0;

    if (Math.abs(totalSelected - expenseAmount) > 0.01) {
      setAmountWarningModal({ open: true, diff: totalSelected - expenseAmount, selected: totalSelected, expenseAmt: expenseAmount });
      return;
    }
    doSave();
  };
          ...
          <div className="text-sm text-slate-600 space-y-1">
            ...
            {amountMatch !== null && (
              <div className={`font-bold flex items-center gap-2 ${amountMatch ? "text-green-600" : "text-red-600"}`}>
                ...
                {amountMatch ? " ✓ תואם לתנועה" : ` ✗ לא תואם (תנועה: ${formatCurrency(expenseAmount)})`}
              </div>
            )}
          </div>
          <div className="flex items-center gap-3">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
            ...
            <div className="px-6 py-5 space-y-3 text-right">
              <p className="text-slate-700">
                ...
                <span className="font-bold text-slate-900">{formatCurrency(amountWarningModal.expenseAmt)}</span>.
              </p>
            ...
            <div className="px-6 pb-5 flex gap-3 justify-end">
              <button
                onClick={() => setAmountWarningModal({ open: false, diff: 0, selected: 0, expenseAmt: 0 })}
              ...
              <button
                onClick={() => {
                  setAmountWarningModal({ open: false, diff: 0, selected: 0, expenseAmt: 0 });
                  doSave();
                }}
                disabled={loading}
